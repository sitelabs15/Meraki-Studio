...
const fieldClass =
  "w-full min-h-12 rounded-sm border border-border/80 bg-surface-deep/90 px-4 py-3 text-sm text-ivory placeholder:text-ash/60 transition-all duration-200 focus:border-ivory focus:bg-surface focus:outline-none focus:ring-1 focus:ring-ivory/30";
...
export function BookingForm() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const formFields = formRef.current?.querySelectorAll<HTMLElement>(".form-item-anim");
      if (formFields) {
        gsap.set(formFields, { y: 16, opacity: 0 });
        gsap.to(formFields, {
          y: 0,
          opacity: 1,
          duration: 0.7,
      ...
      // Magnetic effect on submit button for desktop
      if (!isTouch && buttonRef.current) {
        const btn = buttonRef.current;
        const setBtnX = gsap.quickTo(btn, "x", { duration: 0.35, ease: "power2.out" });
        const setBtnY = gsap.quickTo(btn, "y", { duration: 0.35, ease: "power2.out" });
    ...
    >
      <div className="shell grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
        <div>
          ...
          <div className="mt-8 border-l-2 border-border/80 pl-4 space-y-3">
            <p className="text-sm leading-relaxed text-ash">
              {booking.disclaimer}
            </p>
            <p className="text-xs uppercase tracking-[0.16em] text-ivory/60">
              Puebla, México · Citas personalizadas
            </p>
          </div>
        </div>
        ...
        >
          <div className="form-item-anim grid gap-6 sm:grid-cols-2">
          ...
          <fieldset className="form-item-anim">
          ...
          <div className="form-item-anim flex flex-col gap-4">
            <button
              ref={buttonRef}
              type="submit"
